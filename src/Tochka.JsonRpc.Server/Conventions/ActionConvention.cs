...
public class ActionConvention : IActionModelConvention
{
    ...
    public void Apply(ActionModel action)
    {
        ...
        if (!Utils.IsJsonRpcController(action.Controller.ControllerType))
        {
            return;
        }

        var methodOptions = MakeOptions(action);
        var methodMetadata = GetMethodMetadata(action, methodOptions);
        SetAttributes(action, methodOptions);
        ...
        action.Properties[typeof(MethodMetadata)] = methodMetadata;
    }
    ...
    internal void SetAttributes(ActionModel actionModel, JsonRpcMethodOptions methodOptions)
    {
        ...
        var existingEndpointMetadata = actionModel.Selectors.SelectMany(x => x.EndpointMetadata)
            .Concat(actionModel.Controller.Selectors.SelectMany(x => x.EndpointMetadata))
            .ToList();
