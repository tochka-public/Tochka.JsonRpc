...
using ApiDescriptionExtensions = Swashbuckle.AspNetCore.SwaggerGen.ApiDescriptionExtensions;
    ...
    public class SwaggerGenerator : ISwaggerProvider
    {
        ...
        public OpenApiDocument GetSwagger(string documentName, string host = null, string basePath = null)
        {
            ...
            var applicableApiDescriptions = _apiDescriptionsProvider.ApiDescriptionGroups.Items
                ...
                .Where(apiDesc => !(_options.IgnoreObsoleteActions && ApiDescriptionExtensions.CustomAttributes(apiDesc).OfType<ObsoleteAttribute>().Any()))
        ...
        private OpenApiPaths GeneratePaths(IEnumerable<ApiDescription> apiDescriptions, SchemaRepository schemaRepository)
        {
            var apiDescriptionsByPath = apiDescriptions
                ...
                .GroupBy<ApiDescription, string>(apiDesc => ApiDescriptionExtensions.RelativePathSansQueryString(apiDesc));
        ...
        private OpenApiOperation GenerateOperation(ApiDescription apiDescription, SchemaRepository schemaRepository)
        {
            try
            {
                var operation = new OpenApiOperation
                {
                    ...
                    Deprecated = ApiDescriptionExtensions.CustomAttributes(apiDescription).OfType<ObsoleteAttribute>().Any()
                };

                ApiDescriptionExtensions.TryGetMethodInfo(apiDescription, out MethodInfo methodInfo);
            ...
            var applicableApiParameters = apiDescription.ParameterDescriptions
                .Where(apiParam =>
                {
                    return (!apiParam.IsFromBody() && !apiParam.IsFromForm())
                           && (!ApiParameterDescriptionExtensions.CustomAttributes(apiParam).OfType<BindNeverAttribute>().Any())
        ...
        private OpenApiParameter GenerateParameter(ApiParameterDescription apiParameter, SchemaRepository schemaRepository)
        {
            ...
            var isRequired = (apiParameter.IsFromPath())
                             || ApiParameterDescriptionExtensions.CustomAttributes(apiParameter).Any(attr => RequiredAttributeTypes.Contains(attr.GetType()));

            var schema = (apiParameter.ModelMetadata != null)
                ? _schemaGenerator.GenerateSchema(
                    apiParameter.ModelMetadata.ModelType,
                    schemaRepository,
                    ApiParameterDescriptionExtensions.PropertyInfo(apiParameter),
                    ApiParameterDescriptionExtensions.ParameterInfo(apiParameter))
        ...
        private OpenApiRequestBody GenerateRequestBodyFromBodyParameter(ApiDescription apiDescription, SchemaRepository schemaRepository, ApiParameterDescription bodyParameter)
        {
            ...
            var isRequired = ApiParameterDescriptionExtensions.CustomAttributes(bodyParameter).Any(attr => RequiredAttributeTypes.Contains(attr.GetType()));

            var schema = _schemaGenerator.GenerateSchema(
                bodyParameter.ModelMetadata.ModelType,
                schemaRepository,
                ApiParameterDescriptionExtensions.PropertyInfo(bodyParameter),
                ApiParameterDescriptionExtensions.ParameterInfo(bodyParameter));
        ...
        private IEnumerable<string> InferRequestContentTypes(ApiDescription apiDescription)
        {
            ...
            var explicitContentTypes = ApiDescriptionExtensions.CustomAttributes(apiDescription).OfType<ConsumesAttribute>()
            ...
            foreach (var formParameter in formParameters)
            {
                ...
                var schema = (formParameter.ModelMetadata != null)
                    ? _schemaGenerator.GenerateSchema(
                        ...
                        schemaRepository,
                        ApiParameterDescriptionExtensions.PropertyInfo(formParameter),
                        ApiParameterDescriptionExtensions.ParameterInfo(formParameter))
                    : new OpenApiSchema {Type = "string"};

                properties.Add(name, schema);

                if (formParameter.IsFromPath() || ApiParameterDescriptionExtensions.CustomAttributes(formParameter).Any(attr => RequiredAttributeTypes.Contains(attr.GetType())))
        ...
        private IEnumerable<string> InferResponseContentTypes(ApiDescription apiDescription, ApiResponseType apiResponseType)
        {
            ...
            var explicitContentTypes = ApiDescriptionExtensions.CustomAttributes(apiDescription).OfType<ProducesAttribute>()
                .SelectMany(attr => attr.ContentTypes)
                .Distinct();
            if (explicitContentTypes.Any()) return explicitContentTypes;
