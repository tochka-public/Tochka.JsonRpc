...
namespace Tochka.JsonRpc.V1.Server
{
    internal static class Utils
    {
        public static bool IsJsonRpcController(TypeInfo typeInfo)
        {
            return typeof(JsonRpcController).IsAssignableFrom(typeInfo);
        }
        ...
        public static TAttribute GetAttribute<TAttribute>(ParameterModel parameterModel)
            where TAttribute : Attribute
        {
            return parameterModel.Attributes.OfType<TAttribute>().SingleOrDefault();
        }
        ...
        /// <returns></returns>
        public static bool IsGoodHttpCode(int code)
        {
